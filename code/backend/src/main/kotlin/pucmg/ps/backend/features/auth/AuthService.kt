...
class AuthService(
    ...
    fun login(request: LoginRequest): Pair<AuthResponse, String>{
        ...
        val permissions = user.permissions.map { it.name }.toSet()

        val accessToken = jwtService.generateToken(user.email, permissions)
        val rawRefreshToken = jwtService.generateToken(user.email, permissions)

        val refreshToken = RefreshTokenEntity(
            tokenHash = hash(rawRefreshToken),
            expiresAt = Instant.now().plusSeconds(7 * 24 * 60 * 60), // Expiracao em 7 dias
            user = user
        )
    ...
    fun refresh(rawRefreshToken: String): Pair<AuthResponse, String> {
        val stored = refreshTokenDao.findByTokenHash(hash(rawRefreshToken))
        ...
        val newAccessToken = jwtService.generateToken(user.email, permissions)
        val newRawRefreshToken = jwtService.generateRefreshToken(user.email, permissions)
        ...
        return Pair(response, newRawRefreshToken)
    }
